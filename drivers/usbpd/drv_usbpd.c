...
static void Delay_Us(int us) {
    for (int i = 0; i < us; ++i) {
        volatile uint32_t delaytimer = 100;
        while(delaytimer--);
    }
}
...
uint8_t  Tmr_Ms_Cnt_Last;                                                         /* System timer millisecond timing final value */
uint8_t  Tmr_Ms_Dlt;                                                              /* System timer millisecond timing this interval value */
...
/* SrcCap Table */
uint8_t SrcCap_5V3A_Tab[ 4 ]  = { 0X2C, 0X91, 0X01, 0X3E };
 ...
 * @return  none
 */
static void PD_PDO_Analyse( uint8_t pdo_idx, uint8_t *srccap, UINT16 *current, UINT16 *voltage )
{
    uint32_t temp32;
...
static void PD_Main_Proc( )
{
    ...
    if( PD_Ctl.Flag.Bit.Msg_Recvd )
    {
        ...
        switch( pd_header )
        {
            ...
            default:
                rt_kprintf("Unsupported Command\r\n");
                break;
        }
...
void rt_usbpd_process(void) {
    ...
    uint32_t det = rt_tick_get();
    if (last_det) PD_Ctl.Det_Timer += det - last_det;
    last_det = det;

    if( PD_Ctl.Det_Timer > 4 )
    {
        //rt_kprintf("PD_Ctl.Det_Timer > 4\r\n");
        PD_Ctl.Det_Timer = 0;
        PD_Det_Proc( );
        //rt_kprintf("exit PD_Det_Proc\r\n");
    }
